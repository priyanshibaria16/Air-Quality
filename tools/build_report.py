...
def metric_after(r: dict, name: str):
    """One 'After Cleaning' value from the before/after quality table."""
    for row in r["quality_comparison"]:
        if row["Metric"] == name:
            return row["After Cleaning"]
    raise KeyError(name)


def dax_measure_count() -> int:
    """Count the measure definitions in `dashboard/DAX_measures.dax`.

    The report, the slide deck and `dashboard/KPI_DEFINITIONS.md` all quote this
    number, so it is counted from the file rather than typed: a measure added
    later cannot leave the three documents disagreeing.
    """
    path = C.DASHBOARD_DIR / "DAX_measures.dax"
    if not path.exists():
        return 0
...
def export_width() -> int:
    """Columns of the exported fact table - header only, the file is not loaded."""
    if not C.CLEANED_CSV.exists():
        return 0
    return len(pd.read_csv(C.CLEANED_CSV, nrows=0).columns)


def evidence_counts(r: dict) -> dict:
    """How many insights are labelled 'measured' versus 'interpretation'."""
    return pd.Series([i["Evidence Strength"] for i in r["insights"]]
...
def abstract(b: D.DocxBuilder, r: dict) -> None:
    ...
    text = (
        ...
        f"the derived columns take it from {ds['columns']} to {n(export_width(), 0)} "
        f"columns in the exported analytical table. K-Means settled on "
    ...
    print(f"abstract words: {words}")   # checked by tools/verify_report.py; not printed
                                       # into the submitted document itself
...
def dataset_section(b: D.DocxBuilder, r: dict) -> None:
    ...
    b.para(f"The raw table has one row per city per day and {ds['columns']} columns. "
           "The data dictionary below was generated from the file itself, not copied "
           "from a description of it.", align="justify")
...
def preprocessing(b: D.DocxBuilder, r: dict) -> None:
    ...
    b.para(f"The important number is the one that did not change: {n(ds['rows'], 0)} rows "
           ...
           f"imputed and none was discarded. The column count rises from {ds['columns']} "
           f"to {n(metric_after(r, 'Total Columns'), 0)} because cleaning appends a "
           f"normalised `Date` key and one range-check flag per pollutant, not because "
           f"data was invented; feature engineering then adds the "
           f"{len(r['feature_inventory'])} catalogued features of section 15.4, and the "
           f"table exported for the dashboard carries {n(export_width(), 0)} columns "
           f"once the mining results are attached.",
           align="justify")
    ...
    b.table(log, caption=f"Complete cleaning log ({len(log)} steps) from "
            "`cleaning_log.csv`.", font_size=7.5, max_rows=len(log))
    ...
    b.para(f"The cleaned {n(metric_after(r, 'Total Columns'), 0)}-column table is the "
           "input to feature engineering, which produces the modelling table. Each "
           "catalogued feature and its purpose is recorded in "
    ...
    b.table(pd.DataFrame(r["feature_inventory"]),
            ...
            font_size=7.5, max_rows=len(r["feature_inventory"]))
...
def powerbi(b: D.DocxBuilder, r: dict) -> None:
    ...
    b.para(f"{dax_measure_count()} DAX measures are defined in "
           "`dashboard/DAX_measures.dax`, grouped into "
...
def results_section(b: D.DocxBuilder, r: dict) -> None:
    ...
    b.table([["Result", "Value", "Source"]] + [
        ...
        ["Insights generated", f"{len(r['insights'])} statements, each with its evidence",
         "insights.csv"],
        ["Dashboard", f"1 fact table + 3 dimensions, {dax_measure_count()} DAX measures, "
         "5 pages, "
...
def conclusion(b: D.DocxBuilder, r: dict) -> None:
    ...
    b.para(f"This project took a daily air-quality panel of {n(ds['rows'], 0)} records "
           ...
           f"five-page Power BI dashboard with documented KPIs and "
           f"{dax_measure_count()} DAX measures. The "
